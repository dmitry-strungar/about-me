# Дмитрий Струнгарь - Аналитик данных

## Обо мне

Я - Аналитик данных с практическим опытом в анализе данных, SQL, Python и BI-инструментах. Моя задача - преобразовывать сырые данные в понятные инсайты и интерактивные дашборды для поддержки принятия бизнес-решений.

Я прошёл программу профессиональной переподготовки по BI-аналитике, в рамках которой разрабатывал аналитические решения, включая продуктовую аналитику, сегментацию клиентов, SQL-анализ и управленческие дашборды.

Моё портфолио демонстрирует: подготовку данных, расчет метрик, анализ и визуализацию.

В настоящее время я ищу позицию Аналитика данных, где смогу применять аналитические навыки и продолжать развиваться в коммерческой среде.


В репозитории имеется свидетельство о прохождении курса.  

---

## Технические навыки

**BI и визуализация**
- Power BI (моделирование данных, DAX, Power Query)
- Tableau
- Yandex DataLens

**Анализ данных**
- SQL (PostgreSQL): JOIN, CTE, оконные функции, агрегирование
- Python: Pandas, NumPy, Matplotlib, Seaborn
- Excel (аналитические расчеты и подготовка данных)

**Аналитика и методы**
- Продуктовая аналитика (конверсия, retention, воронки)
- Сегментация клиентов и анализ LTV
- A/B-тестирование и проверка гипотез
- Анализ бизнес-метрик и эффективности

---

## Проекты портфолио

Моё портфолио включает проекты в области:

- Продуктовой аналитики (воронка, retention, монетизация)
- BI-дашбордов для бизнес- и управленческой отчетности
- SQL-анализа бизнес-операций
- Клиентской аналитики и анализа LTV
- Статистического анализа в Python
- Рыночной и бизнес-аналитики

Репозитории портфолио доступны в данном профиле.

---

## Образование

Профессиональная переподготовка - BI-аналитик  
Нетология, 2025-2026  

---

## Контакты

Email: dmitry.strungar@gmail.com  

Telegram: https://t.me/dmitry_strungar  

Локация: Россия

---

## Карьерная цель

Получить профессиональную работу в области аналитики данных, где я смогу применять инструменты анализа и визуализации данных для решения бизнес‑задач и поддержки принятия решений.
